...
// QueryFloor 实现 gRPC 接口
func (server *Server) QueryFloor(ctx context.Context, req *pb.QueryFloorRequest) (*pb.QueryFloorResponse, error) {
	_, err := server.authorizeUser(ctx)
	if err != nil {
		return nil, unauthenticatedError(err)
	}
	
	if req.GetAreaId() == "" || req.GetBuildingCode() == "" {
		return nil, status.Errorf(codes.InvalidArgument, "缺少参数 areaId 或 buildingCode")
	}
